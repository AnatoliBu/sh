...
- [Playwright Best Practices](cards/playwright-best-practices.md)
- [QA Skills Agent Catalog](cards/qa-skills-agent-catalog.md)
- [Agentic QA Boilerplate](cards/agentic-qa-boilerplate.md)
...
Return exactly one **canonical decision label**. Это единый enum для ВСЕГО скилла —
decision-flow, `rules.md`, `workflows.md` используют этот же набор, не свои варианты:

```text
DO               — делать сейчас
DO (WITH RISK)   — делать с явно принятым риском
SPIKE FIRST      — сначала маленький проверочный spike
LATER            — отложить, с триггером возврата
DO NOT DO        — не делать
MANUAL FOR NOW   — оставить ручным/exploratory пока
```
...
DO / DO (WITH RISK) / SPIKE FIRST / LATER / DO NOT DO / MANUAL FOR NOW   (canonical enum)
